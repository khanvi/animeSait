		...
		<div class="bg-light" style="width: 880px">
			<div style="height: 50px; text-align: center; margin-bottom: 10px">
					<ul class="row main">
						...
						<li><a id="activelink3" href="">ПРОФИЛЬ</a></li>
			...
			<div class="row" style="padding: 10px 20px; margin-bottom: 10px; width: 880px">
				<?php
				$query_bookmark = mysqli_query($con,"SELECT * FROM bookmark INNER JOIN anime ON 
					bookmark.bookmark_id_anime = anime.anime_id
						WHERE bookmark.bookmark_id_users='".$_GET['user']. "'");
				$number_bookmark=$query_bookmark->num_rows;
				if($_GET['user'] > 0){
					for($i=0;$i<$number_bookmark;$i++){
						$res_bookmark = $query_bookmark->fetch_assoc();
				?>
				<a <?php echo 'href="anime.php?anime_id='. $res_bookmark['anime_id'] . '&user='.$_GET['user'].'"' ?> class="pp" style="margin-left: auto; margin-right: auto; text-decoration: none">
					<div style="width: 270px; margin-bottom: 15px; height: 390px; ">
						<div class="anime_info">
							<h5 class="anime_name">
								<?php echo $res_bookmark['anime_name'] ?>
							</h5>
							<span class="anime_text">
								<?php echo $res_bookmark['anime_text'] ?>
							</span>
						</div>
						<img border="0" <?php echo 'src="' . $res_bookmark['anime_img'] . ' "' ?> width="270" height="390" class="anime_kk">
					</div>
				</a>
				<?php
					}
				...
				echo 
				'<div class="page-header bg-warning" style="margin: auto"><h2 style="text-align: center; padding: 10px;">
				У вас нет закладок</h2></div>'; } ?>
			</div>
		</div>
